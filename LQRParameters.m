...
Y0dict = solve(
          ...
          F0(8,1) == 0,
          F0(9,1) == 0,
          ddx, ddtheta1, ddtheta2, ddx1, ddx2, ddy1, ddy2, T1, T2);


#Y0 = arrayfun(@(i) double(Y0.(char(Y(i)))),1:9)'; % Extract numeric values
Y0 = zeros(9,1);
Y0(1) = double(Y0dict.ddx);
Y0(2) = double(Y0dict.ddtheta1);
Y0(3) = double(Y0dict.ddtheta2);
Y0(4) = double(Y0dict.ddx1);
Y0(5) = double(Y0dict.ddx2);
Y0(6) = double(Y0dict.ddy1);
Y0(7) = double(Y0dict.ddy2);
Y0(8) = double(Y0dict.T1);
Y0(9) = double(Y0dict.T2);
...
Q = 1000*eye(6);
Q(1,1)=1; % Put high priority on minimizing x (cart position).
Q(4,4)=1;
G = lqr(A,B,Q,1)
